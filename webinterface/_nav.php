...
if (($job_check = $mysqlcon->query("SELECT * FROM `$dbname`.`job_check`")->fetchAll(PDO::FETCH_UNIQUE | PDO::FETCH_ASSOC)) === false) {
    ...
    if ((time() - $job_check['last_update']['timestamp']) < 259200 && ! isset($_SESSION[$rspathhex.'upinfomsg'])) {
        if (! isset($err_msg)) {
            $err_msg = '<i class="fas fa-info-circle"></i><span class="item-margin">'.sprintf($lang['upinf2'], date('Y-m-d H:i', $job_check['last_update']['timestamp']), '</span><i class="fas fa-book"></i><span class="item-margin"><a href="//ts-ranksystem.com/?changelog" target="_blank">', '</a></span>');
            $err_lvl = 1;
...
if (isset($_POST['switchexpert']) && isset($_SESSION[$rspathhex.'username']) && hash_equals($_SESSION[$rspathhex.'username'], $cfg['webinterface_user']) && hash_equals($_SESSION[$rspathhex.'password'], $cfg['webinterface_pass'])) {
    if ($_POST['switchexpert'] == 'check') {
        $cfg['webinterface_advanced_mode'] = 1;
    } else {
        $cfg['webinterface_advanced_mode'] = 0;
    }
...
if ((array_key_exists('webinterface_fresh_installation', $cfg) && $cfg['webinterface_fresh_installation'] != 1) || ! array_key_exists('webinterface_fresh_installation', $cfg)) {
    echo '<li'.(basename($_SERVER['SCRIPT_NAME']) == 'db.php' ? ' class="active expertelement">' : ' class="expertelement">'); ?>
						...
						<li>
							...
							<?php echo '<ul id="rank" class="'.(in_array(basename($_SERVER['SCRIPT_NAME']), ['core.php', 'rank.php', 'boost.php', 'except.php', 'msg.php']) ? 'in collapse">' : 'collapse">'); ?>
						...
						<li>
							...
							<?php echo '<ul id="stats" class="'.(in_array(basename($_SERVER['SCRIPT_NAME']), ['stats.php', 'ranklist.php', 'imprint.php']) ? 'in collapse">' : 'collapse">'); ?>
						...
						<li>
							...
							<?php echo '<ul id="admin" class="'.(in_array(basename($_SERVER['SCRIPT_NAME']), ['admin_addtime.php', 'admin_remtime.php', 'admin_delclient.php', 'reset.php', 'export.php']) ? 'in collapse">' : 'collapse">'); ?>
						...
						<li>
							...
							<?php echo '<ul id="addons" class="'.(in_array(basename($_SERVER['SCRIPT_NAME']), ['addon_assign_groups.php', 'api.php', 'addon_channelinfo_toplist.php']) ? 'in collapse">' : 'collapse">'); ?>
