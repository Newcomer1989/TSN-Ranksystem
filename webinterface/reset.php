...
try {
    ...
    function reset_status($lang, $job_check, $check = null)
    {
        ...
        switch([$job_check['reset_user_time']['timestamp'], $job_check['reset_user_delete']['timestamp']]) {
            case [0, 1]:
                if ($check == 1) {
                    $err_msg .= $lang['wihladmrs16'].' ('.$lang['wisupidle'].': '.$lang['wihladm312'].')<br>';
                    break;
                } else {
                    $err_msg .= $lang['wihladmrs1'].' ('.$lang['wisupidle'].': '.$lang['wihladm312'].')<br>';
                    break;
                }
            case [0, 2]:
                $err_msg .= '<span class="alert-info">'.$lang['wihladmrs2'].'</span> ('.$lang['wisupidle'].': '.$lang['wihladm312'].')<br>';
                break;
            case [0, 3]:
                $err_msg .= '<span class="alert-danger">'.$lang['wihladmrs3'].'</span> ('.$lang['wisupidle'].': '.$lang['wihladm312'].')<br>';
                break;
            case [0, 4]:
                $err_msg .= '<span class="alert-success">'.$lang['wihladmrs4'].'</span> ('.$lang['wisupidle'].': '.$lang['wihladm312'].')<br>';
                break;
            case [1, 0]:
                if ($check == 1) {
                    $err_msg .= $lang['wihladmrs16'].' ('.$lang['wisupidle'].': '.$lang['wihladm311'].')<br>';
                    break;
                } else {
                    $err_msg .= $lang['wihladmrs1'].' ('.$lang['wisupidle'].': '.$lang['wihladm311'].')<br>';
                    break;
                }
            case [2, 0]:
                $err_msg .= '<span class="alert-info">'.$lang['wihladmrs2'].'</span> ('.$lang['wisupidle'].': '.$lang['wihladm311'].')<br>';
                break;
            case [3, 0]:
                $err_msg .= '<span class="alert-danger">'.$lang['wihladmrs3'].'</span> ('.$lang['wisupidle'].': '.$lang['wihladm311'].')<br>';
                break;
            case [4, 0]:
                $err_msg .= '<span class="alert-success">'.$lang['wihladmrs4'].'</span> ('.$lang['wisupidle'].': '.$lang['wihladm311'].')<br>';
                break;
            default:
        ...
        switch($job_check['reset_group_withdraw']['timestamp']) {
            case 1:
                if ($check == 1) {
                    $err_msg .= $lang['wihladmrs16'].'<br>';
                    break;
                } else {
                    $err_msg .= $lang['wihladmrs1'].'<br>';
                    break;
                }
            case 2:
                $err_msg .= '<span class="alert-info">'.$lang['wihladmrs2'].'</span><br>';
                break;
            case 3:
                $err_msg .= '<span class="alert-danger">'.$lang['wihladmrs3'].'</span><br>';
                break;
            case 4:
                $err_msg .= '<span class="alert-success">'.$lang['wihladmrs4'].'</span><br>';
                break;
            default:
                $err_msg .= '<span class="alert-secondary"><i>'.$lang['wihladmrs0'].'</i></span><br>';
        }
    ...
    if ($job_check['reset_user_time']['timestamp'] != 0 || $job_check['reset_user_delete']['timestamp'] != 0 || $job_check['reset_group_withdraw']['timestamp'] != 0 || $job_check['reset_webspace_cache']['timestamp'] != 0 || $job_check['reset_usage_graph']['timestamp'] != 0) {
        $err_msg = '<b>'.$lang['wihladmrs'].':</b><br><br><pre>';
        $err_lvl = 2;
    ...
    if (isset($_POST['confirm']) && isset($db_csrf[$_POST['csrf_token']])) {
        ...
        if ($job_check['reset_user_time']['timestamp'] != 0 || $job_check['reset_user_delete']['timestamp'] != 0 || $job_check['reset_group_withdraw']['timestamp'] != 0 || $job_check['reset_webspace_cache']['timestamp'] != 0 || $job_check['reset_usage_graph']['timestamp'] != 0) {
            ...
            if (isset($_POST['reset_group_withdraw'])) {
                $_POST['reset_group_withdraw'] = $job_check['reset_group_withdraw']['timestamp'] = 1;
            } else {
                $_POST['reset_group_withdraw'] = $job_check['reset_group_withdraw']['timestamp'] = 0;
            }
            if (isset($_POST['reset_webspace_cache'])) {
                $_POST['reset_webspace_cache'] = $job_check['reset_webspace_cache']['timestamp'] = 1;
            } else {
                $_POST['reset_webspace_cache'] = $job_check['reset_webspace_cache']['timestamp'] = 0;
            }
            if (isset($_POST['reset_usage_graph'])) {
                $_POST['reset_usage_graph'] = $job_check['reset_usage_graph']['timestamp'] = 1;
            } else {
                $_POST['reset_usage_graph'] = $job_check['reset_usage_graph']['timestamp'] = 0;
            }
            if (isset($_POST['reset_stop_after'])) {
                $_POST['reset_stop_after'] = $job_check['reset_stop_after']['timestamp'] = 1;
            } else {
                $_POST['reset_stop_after'] = $job_check['reset_stop_after']['timestamp'] = 0;
            }

            if ($_POST['reset_group_withdraw'] == 0) {
                $delay = 0;
            } else {
                $delay = ($cfg['teamspeak_query_command_delay'] / 1000000) + 0.05;
            }
            if ($_POST['reset_webspace_cache'] == 0) {
                $cache_needed_time = 0;
            } else {
                $cache_needed_time = $stats_server['total_user'] / 10 * 0.005;
            }
