...
try {
    ...
    if (($cfg['webinterface_access_last'] + 1) >= time()) {
        ...
        $err_msg = sprintf($lang['wirtpw1'], '<a href="https://github.com/Newcomer1989/TSN-Ranksystem/wiki#reset-password-webinterface" target="_blank">https://github.com/Newcomer1989/TSN-Ranksystem/wiki#reset-password-webinterface</a>');
        $err_lvl = 3;
        ...
        try {
            ...
            try {
                ...
                $ts3->selfUpdate(['client_nickname' => 'Ranksystem - Reset Password']);
            } catch (Exception $e) {
            }
            ...
            try {
                        ...
                        if ($client['connection_client_ip'] == getclientip()) {
                            ...
                            if ($mysqlcon->exec("INSERT INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('webinterface_pass','{$cfg['webinterface_pass']}'),('webinterface_access_last','0') ON DUPLICATE KEY UPDATE `value`=VALUES(`value`)") === false) {
                                ...
                                try {
                                    ...
                                    $err_msg .= sprintf($lang['wirtpw5'], '<a href="http'.(! empty($_SERVER['HTTPS']) ? 's' : '').'://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['SCRIPT_NAME']).'/">', '</a>');
                                    $err_lvl = 1;
                                    enter_logfile(3, sprintf($lang['wirtpw6'], getclientip()));
                                } catch (Exception $e) {
                                    $err_msg .= $lang['errorts3'].$e->getCode().': '.$e->getMessage();
                                    $err_lvl = 3;
                                }
                            }
                        }
                ...
                if (! isset($checkuuid)) {
                    $err_msg = $lang['wirtpw2'];
                    $err_lvl = 3;
                } elseif (! isset($checkip)) {
                    $err_msg = $lang['wirtpw3'];
                    $err_lvl = 3;
                }
            } catch (Exception $e) {
                $err_msg = $lang['errorts3'].$e->getCode().': '.$e->getMessage();
