...
try {
    ...
    if (isset($_POST['confirm']) && isset($db_csrf[$_POST['csrf_token']])) {
        ...
        if ($mysqlcon->exec("INSERT INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('database_export','1') ON DUPLICATE KEY UPDATE `timestamp`=VALUES(`timestamp`); DELETE FROM `$dbname`.`csrf_token` WHERE `token`='{$_POST['csrf_token']}'") === false) {
            ...
            if (($snapshot = $mysqlcon->query("SELECT COUNT(*) AS `count` from `$dbname`.`user_snapshot`")->fetch()) === false) {
            } else {
								...
								<div class="form-group">
									...
									<div class="panel-body">
                                        ...
                                        foreach (scandir($GLOBALS['logpath']) as $file) {
                                            ...
                                            if (is_dir($GLOBALS['logpath'].$file)) {
                                                continue;
                                            }
                                            if (substr($file, 0, 10) != 'db_export_') {
                                                continue;
                                            }
                                            if (substr($file, -4, 4) != '.zip' && substr($file, -4, 4) != '.sql') {
                                                continue;
                                            }
                                            ?>
											<div class="col-sm-6">
