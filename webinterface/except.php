...
try {
    ...
    if (isset($_POST['update']) && isset($db_csrf[$_POST['csrf_token']])) {
        ...
        if (isset($_POST['rankup_excepted_remove_group_switch'])) {
            $cfg['rankup_excepted_remove_group_switch'] = 1;
        } else {
									...
									<div class="panel-body">
										<div class="form-group expertelement">
											...
											<div class="col-sm-8">
												...
												<?php
                                                echo '<option data-icon="fas fa-stopwatch" value="0"';
    if ($cfg['rankup_excepted_mode'] == '0') {
        echo ' selected=selected';
    } echo '><span class="item-margin">',$lang['wiexres1'],'</span></option>';
    echo '<option data-icon="fas fa-pause" value="1"';
    if ($cfg['rankup_excepted_mode'] == '1') {
        echo ' selected=selected';
    } echo '><span class="item-margin">',$lang['wiexres2'],'</span></option>';
    echo '<option data-icon="fas fa-sync" value="2"';
    if ($cfg['rankup_excepted_mode'] == '2') {
        echo ' selected=selected';
    } echo '><span class="item-margin">',$lang['wiexres3'],'</span></option>';
    ?>
												</select>
											</div>
										</div>
    ...
    foreach ($user_arr as $user) {
        if ($cfg['rankup_excepted_unique_client_id_list'] != null && array_key_exists($user['uuid'], $cfg['rankup_excepted_unique_client_id_list'])) {
            $selected = ' selected';
        } else {
            $selected = '';
        }
    ...
    foreach ($groupslist as $groupID => $groupParam) {
        if ($cfg['rankup_excepted_group_id_list'] != null && array_key_exists($groupID, $cfg['rankup_excepted_group_id_list'])) {
            $selected = ' selected';
        } else {
            $selected = '';
        }
