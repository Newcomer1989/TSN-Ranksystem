...
try {
    ...
    if (version_compare(PHP_VERSION, '5.5.0', '<')) {
        $err_msg = sprintf($lang['insterr4'], PHP_VERSION);
        $err_lvl = 3;
        $dis_login = 1;
    }
    ...
    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
        if (! in_array('com_dotnet', get_loaded_extensions())) {
            unset($err_msg);
            $err_msg = sprintf($lang['insterr2'], 'PHP COM extension (php_com_dotnet.dll)', '//php.net/manual/en/book.com.php', get_cfg_var('cfg_file_path'));
            $err_lvl = 3;
            $dis_login = 1;
        }
    }
    ...
    if (! isset($cfg['webinterface_access_count']) || $cfg['webinterface_access_count'] != null) {
        $cfg['webinterface_access_count'] = 0;
    }
    if (! isset($cfg['webinterface_access_last']) || $cfg['webinterface_access_last'] != null) {
        $cfg['webinterface_access_last'] = 0;
    }
