...
function run_bot(&$cfg)
{
    ...
    try {
                ...
                if (version_compare($ts3version['version'], '3.4.0', '>=') || version_compare($ts3version['version'], '3.0.0', '<=')) {
                    ...
                    for ($updcld = 0; $updcld < 10; $updcld++) {
                        try {
                            usleep($cfg['teamspeak_query_command_delay']);
                            if ($updcld == 0) {
                                $ts3server->selfUpdate(['client_nickname' => $cfg['teamspeak_query_nickname']]);
                            } else {
                                $ts3server->selfUpdate(['client_nickname' => $cfg['teamspeak_query_nickname'].$updcld]);
                            }
                            break;
                        } catch (Exception $e) {
        ...
        $looptime = $cfg['temp_count_laps'] = $cfg['temp_whole_laptime'] = $cfg['temp_count_laptime'] = 0;
        ...
        if (in_array($e->getCode(), [110, 257, 258, 1024, 1026, 1031, 1032, 1033, 1034, 1280, 1793])) {
        ...
        if ($cfg['temp_last_botstart'] < (time() - 10)) {
            ...
            $check_db_arr = array_flip(['HY000', 10054, 70100]);
