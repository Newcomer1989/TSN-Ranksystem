    ...
    if (isset($addons_config['channelinfo_toplist_active']['value']) && $addons_config['channelinfo_toplist_active']['value'] == '1') {
        if ($addons_config['channelinfo_toplist_lastupdate']['value'] < ($nowtime - $addons_config['channelinfo_toplist_delay']['value'])) {
            switch($addons_config['channelinfo_toplist_modus']['value']) {
                case 1: $filter = 'ORDER BY (`count_week`-`idle_week`)';
                    break;
                case 2: $filter = 'ORDER BY `count_week`';
                    break;
                case 3: $filter = 'ORDER BY (`count_month`-`idle_month`)';
                    break;
                case 4: $filter = 'ORDER BY `count_month`';
                    break;
                case 5: $filter = 'ORDER BY (`count`-`idle`)';
                    break;
                case 6: $filter = 'ORDER BY `count`';
                    break;
            ...
            try {
                ...
                if ($addons_config['channelinfo_toplist_lastdesc']['value'] != $toplist_desc) {
                    try {
                        $ts3->channelGetById($addons_config['channelinfo_toplist_channelid']['value'])->modify(['cid='.$addons_config['channelinfo_toplist_channelid']['value'], 'channel_description='.$toplist_desc]);
