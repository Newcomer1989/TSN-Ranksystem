...
function check_db($mysqlcon, $lang, &$cfg, $dbname)
{
    ...
    {
        $del_folder = ['icons/', 'libs/ts3_lib/Adapter/Blacklist/', 'libs/ts3_lib/Adapter/TSDNS/', 'libs/ts3_lib/Adapter/Update/', 'libs/fonts/'];
        $del_files = ['install.php', 'libs/combined_stats.css', 'libs/combined_stats.js', 'webinterface/admin.php', 'libs/ts3_lib/Adapter/Blacklist/Exception.php', 'libs/ts3_lib/Adapter/TSDNS/Exception.php', 'libs/ts3_lib/Adapter/Update/Exception.php', 'libs/ts3_lib/Adapter/Blacklist.php', 'libs/ts3_lib/Adapter/TSDNS.php', 'libs/ts3_lib/Adapter/Update.php', 'languages/core_ar.php', 'languages/core_cz.php', 'languages/core_de.php', 'languages/core_en.php', 'languages/core_es.php', 'languages/core_fr.php', 'languages/core_it.php', 'languages/core_nl.php', 'languages/core_pl.php', 'languages/core_pt.php', 'languages/core_ro.php', 'languages/core_ru.php', 'webinterface/nav.php', 'stats/nav.php', 'other/session.php'];
    ...
    if ($cfg['version_current_using'] == $cfg['version_latest_available']) {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.1', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('reset_user_time', '0'),('reset_user_delete', '0'),('reset_group_withdraw', '0'),('reset_webspace_cache', '0'),('reset_usage_graph', '0'),('reset_stop_after', '0');") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.4', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('stats_show_maxclientsline_switch', 0)") === false) {
            } else {
                enter_logfile(4, '    [1.3.4] Added new config values.');
            }
            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`groups` MODIFY COLUMN `sgidname` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;") === false) {
            } else {
                enter_logfile(4, '    [1.3.4] Adjusted table groups successfully.');
            }
            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`user` MODIFY COLUMN `name` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;") === false) {
            } else {
                enter_logfile(4, '    [1.3.4] Adjusted table user successfully.');
            }

            if ($mysqlcon->exec("UPDATE `$dbname`.`job_check` SET `timestamp`='".time()."' WHERE `job_name`='last_update';") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.7', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('webinterface_fresh_installation', '0'),('webinterface_advanced_mode', '1')") === false) {
            } else {
            ...
            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`groups` ADD COLUMN `sortid` int(10) NOT NULL default '0', ADD COLUMN `type` tinyint(1) NOT NULL default '0';") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.8', '<')) {
            ...
            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_user` MODIFY COLUMN `base64hash` char(40) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL;") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table stats_user (part 3) successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`server_usage` MODIFY COLUMN `timestamp` int(10) UNSIGNED NOT NULL default '0', MODIFY COLUMN `clients` smallint(5) UNSIGNED NOT NULL default '0', MODIFY COLUMN `channel` smallint(5) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table server_usage successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`job_check` MODIFY COLUMN `timestamp` int(10) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table job_check successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`admin_addtime` MODIFY COLUMN `uuid` char(28) CHARACTER SET utf8 COLLATE utf8_unicode_ci, MODIFY COLUMN `timestamp` int(10) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table admin_addtime successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`user_iphash` MODIFY COLUMN `uuid` char(28) CHARACTER SET utf8 COLLATE utf8_unicode_ci;") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table user_iphash successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_versions` MODIFY COLUMN `count` smallint(5) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table stats_versions successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_platforms` MODIFY COLUMN `count` smallint(5) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table stats_platforms successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_nations` MODIFY COLUMN `nation` char(2) CHARACTER SET utf8 COLLATE utf8_unicode_ci, MODIFY COLUMN `count` smallint(5) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table stats_nations successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`groups` ADD COLUMN `ext` char(3) CHARACTER SET utf8 COLLATE utf8_unicode_ci;") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table groups (part 1) successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`groups` MODIFY COLUMN `sgid` int(10) UNSIGNED NOT NULL default '0', MODIFY COLUMN `icondate` int(10) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table groups (part 2) successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`groups` MODIFY COLUMN `sgidname` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL;") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table groups (part 3) successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`csrf_token` MODIFY COLUMN `timestamp` int(10) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table csrf_token successfully (part 1).');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`csrf_token` MODIFY COLUMN `sessionid` varchar(128) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL;") === false) {
            } else {
                enter_logfile(4, '    [1.3.8] Adjusted table csrf_token successfully (part 2).');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`addon_assign_groups` MODIFY COLUMN `uuid` char(28) CHARACTER SET utf8 COLLATE utf8_unicode_ci;") === false) {
            } else {
            ...
            if ($mysqlcon->exec("UPDATE `$dbname`.`user` SET `idle`=0 WHERE `idle`<0; UPDATE `$dbname`.`user` SET `count`=`idle` WHERE `count`<0;") === false) {
            } else {
                enter_logfile(6, '    [1.3.8] Fix for negative values in table user.');
            }

            if ($mysqlcon->exec("UPDATE `$dbname`.`user_snapshot` SET `idle`=0 WHERE `idle`<0; UPDATE `$dbname`.`user_snapshot` SET `count`=`idle` WHERE `count`<0;") === false) {
            } else {
            ...
            if ($check_snapshot_convert[0]['Field'] == 'timestamp' && $check_snapshot_convert[0]['Field'] != 'id') {
                if ($mysqlcon->exec("DELETE `a` FROM `$dbname`.`user_snapshot` AS `a` CROSS JOIN(SELECT DISTINCT(`timestamp`) FROM `$dbname`.`user_snapshot` ORDER BY `timestamp` DESC LIMIT 1000 OFFSET 121) AS `b` WHERE `a`.`timestamp`=`b`.`timestamp`;") === false) {
                } else {
                    enter_logfile(4, '    [1.3.8] Deleted old values out of the table user_snapshot.');
                }

                if ($mysqlcon->exec("CREATE TABLE `$dbname`.`user_snapshot2` (`id` tinyint(3) UNSIGNED NOT NULL default '0',`cldbid` int(10) UNSIGNED NOT NULL default '0',`count` int(10) UNSIGNED NOT NULL default '0',`idle` int(10) UNSIGNED NOT NULL default '0',PRIMARY KEY (`id`,`cldbid`));") === false) {
                } else {
                ...
                if ($mysqlcon->exec("DROP TABLE `$dbname`.`user_snapshot`;") === false) {
                } else {
                    enter_logfile(4, '    [1.3.8] Dropped old table user_snapshot successfully.');
                }

                if ($mysqlcon->exec("RENAME TABLE `$dbname`.`user_snapshot2` TO `$dbname`.`user_snapshot`;") === false) {
                } else {
                ...
                if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('last_snapshot_id', '{$currentid}'),('last_snapshot_time', '{$lastsnapshot[0]['timestamp']}');") === false) {
                } else {
                    enter_logfile(4, '    [1.3.8] Added new job_check values (part 1).');
                }

                if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('update_groups', '0');") === false) {
                } else {
            ...
            foreach (scandir(substr(dirname(__FILE__), 0, -4).'tsicons/') as $file) {
                if (in_array($file, ['.', '..', 'check.png', 'placeholder.png', 'rs.png', 'servericon.png', '100.png', '200.png', '300.png', '500.png', '600.png'])) {
                    continue;
                }
        ...
        if (version_compare($cfg['version_current_using'], '1.3.9', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('get_avatars', '0'),('calc_donut_chars', '0'),('reload_trigger', '0');") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.10', '<')) {
            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_user` ADD COLUMN `last_calculated` int(10) UNSIGNED NOT NULL default '0';") === false) {
            } else {
                enter_logfile(4, '    [1.3.10] Added new stats_user values.');
            }
            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_user` MODIFY COLUMN `total_connections` MEDIUMINT(8) UNSIGNED NOT NULL default '0';") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.11', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`addons_config` (`param`,`value`) VALUES ('assign_groups_excepted_groupids','');") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.13', '<')) {
            ...
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('database_export', '0'),('update_groups', '0') ON DUPLICATE KEY UPDATE `timestamp`=VALUES(`timestamp`);") === false) {
            } else {
            ...
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('default_session_sametime', 'Strict'),('default_header_origin', ''),('default_header_xss', '1; mode=block'),('default_header_contenttyp', '1'),('default_header_frame', '') ON DUPLICATE KEY UPDATE `value`=VALUES(`value`);") === false) {
            } else {
                enter_logfile(4, '    [1.3.13] Added new cfg_params values.');
            }

            if ($mysqlcon->exec("UPDATE `$dbname`.`user` SET `nation`='XX' WHERE `nation`='';") === false) {
            } else {
                enter_logfile(4, '    [1.3.13] Updated table user.');
            }

            try {
                if ($mysqlcon->exec("DROP INDEX `snapshot_id` ON `$dbname`.`user_snapshot` (`id`)") === false) {
                } else {
                    enter_logfile(4, '    [1.3.13] Dropped unneeded Index snapshot_id on table user_snapshot.');
                }
                if ($mysqlcon->exec("DROP INDEX `snapshot_cldbid` ON `$dbname`.`user_snapshot` (`cldbid`)") === false) {
                } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.14', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('stats_column_default_sort_2', 'rank'),('stats_column_default_order_2', 'asc');") === false) {
            } else {
                enter_logfile(4, '    [1.3.14] Added new cfg_params values.');
            }

            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`addons_config` (`param`,`value`) VALUES ('assign_groups_name','');") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.16', '<')) {
            if ($mysqlcon->exec("INSERT INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('calc_user_removed', '0') ON DUPLICATE KEY UPDATE `timestamp`=VALUES(`timestamp`);") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.18', '<')) {
            if ($mysqlcon->exec("INSERT INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('update_channel', '0') ON DUPLICATE KEY UPDATE `timestamp`=VALUES(`timestamp`);") === false) {
            } else {
                enter_logfile(4, '    [1.3.18] Added new job_check values.');
            }

            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('default_cmdline_sec_switch', '1');") === false) {
            } else {
                enter_logfile(4, '    [1.3.18] Added new cfg_params values.');
            }

            if ($mysqlcon->exec("CREATE TABLE IF NOT EXISTS `$dbname`.`channel` (`cid` int(10) UNSIGNED NOT NULL default '0',`pid` int(10) UNSIGNED NOT NULL default '0',`channel_order` int(10) UNSIGNED NOT NULL default '0',`channel_name` varchar(40) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,PRIMARY KEY (`cid`));") === false) {
            } else {
                enter_logfile(4, '    [1.3.18] Created new table channel successfully.');
            }

            if ($mysqlcon->exec("ALTER TABLE `$dbname`.`addons_config` MODIFY COLUMN `value` varchar(16000) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;") === false) {
            } else {
...
[SIZE=6]Updated: {$LAST_UPDATE_TIME}[/SIZE]', ENT_QUOTES);
        ...
        if (version_compare($cfg['version_current_using'], '1.3.20', '<')) {
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('clean_user_iphash', '0') ON DUPLICATE KEY UPDATE `timestamp`=VALUES(`timestamp`);") === false) {
            } else {
        ...
        if (version_compare($cfg['version_current_using'], '1.3.22', '<')) {
            ...
            if (($check_new_columns = $mysqlcon->query("SHOW COLUMNS FROM `$dbname`.`stats_user` WHERE `field`='count_day'")->fetchAll(PDO::FETCH_ASSOC)) === false) {
            } else {
                if (! isset($check_new_columns[0]['Field'])) {
                    if ($mysqlcon->exec("ALTER TABLE `$dbname`.`stats_user` ADD COLUMN `count_day` mediumint(8) UNSIGNED NOT NULL default '0', ADD COLUMN `idle_day` mediumint(8) UNSIGNED NOT NULL default '0', ADD COLUMN `active_day` mediumint(8) UNSIGNED NOT NULL default '0';") === false) {
                    } else {
                        enter_logfile(4, '    [1.3.22] Adjusted table stats_user successfully.');
                    }
                }
            }
            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`job_check` (`job_name`,`timestamp`) VALUES ('news_html', '".time()."'),('news_bb', '".time()."') ON DUPLICATE KEY UPDATE `timestamp`=VALUES(`timestamp`);") === false) {
            } else {
                enter_logfile(4, '    [1.3.22] Added new job_check values.');
            }

            if ($mysqlcon->exec("INSERT IGNORE INTO `$dbname`.`cfg_params` (`param`,`value`) VALUES ('stats_news_html', '0'),('teamspeak_news_bb', '0') ON DUPLICATE KEY UPDATE `value`=VALUES(`value`);") === false) {
            } else {
    ...
    if (isset($updatedone) && $updatedone === true) {
        return true;
    }
