            ...
            if ($userstats['lastseen'] > ($nowtime - 2678400)) {
                ...
                if (isset($userdata[$userstats['cldbid']]) && $userdata[$userstats['cldbid']] != null) {
                    ...
                    if (isset($userdata[$userstats['cldbid']]) && isset($userdata[$userstats['cldbid']][$keyday]) && $userdata[$userstats['cldbid']][$keyday]['id'] == $dayago) {
                        ...
                        if ($idle_day < 0 || $count_day < $idle_day) {
                            $idle_day = 0;
                        }
                        if ($active_day < 0 || $count_day < $idle_day) {
                            $active_day = 0;
                        }
                    } else {
                    ...
                    if (isset($userdata[$userstats['cldbid']]) && isset($userdata[$userstats['cldbid']][$keyweek]) && $userdata[$userstats['cldbid']][$keyweek]['id'] == $weekago) {
                        ...
                        if ($idle_week < 0 || $count_week < $idle_week) {
                            $idle_week = 0;
                        }
                        if ($active_week < 0 || $count_week < $idle_week) {
                            $active_week = 0;
                        }
                    } else {
                    ...
                    if (isset($userdata[$userstats['cldbid']]) && isset($userdata[$userstats['cldbid']][$keymonth]) && $userdata[$userstats['cldbid']][$keymonth]['id'] == $monthago) {
                        ...
                        if ($idle_month < 0 || $count_month < $idle_month) {
                            $idle_month = 0;
                        }
                        if ($count_month < 0 || $count_month < $idle_month) {
                            $count_month = 0;
                        }
                        if ($active_month < 0 || $count_month < $idle_month) {
                            $active_month = 0;
                        }
                    } else {
                ...
                try {
                    ...
                    if ($clientinfo['client_totalconnections'] > 16777215) {
                        $clientinfo['client_totalconnections'] = 16777215;
                    }
                } catch (Exception $e) {
