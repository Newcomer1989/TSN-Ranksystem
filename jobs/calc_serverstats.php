...
function calc_serverstats($ts3, $mysqlcon, &$cfg, $dbname, $dbtype, $serverinfo, &$db_cache, $phpcommand, $lang)
{
    ...
    if (intval($db_cache['job_check']['calc_donut_chars']['timestamp']) < ($nowtime - 8) || intval($db_cache['job_check']['get_version']['timestamp']) < ($nowtime - 43199)) {
        ...
        $country_array = $platform_array = $count_version_user = [];
        ...
        foreach ($db_cache['all_user'] as $uuid) {
            ...
            if ($uuid['count'] > 0) {
                $total_online_time += $uuid['count'];
            }
            if ($uuid['idle'] > 0) {
                $total_inactive_time += $uuid['idle'];
            }
        }
        ...
        if ($serverinfo['virtualserver_total_ping'] > 32767) {
            $serverinfo['virtualserver_total_ping'] = 32767;
        }
        if (! isset($serverinfo['virtualserver_weblist_enabled']) || $serverinfo['virtualserver_weblist_enabled'] === null) {
            $serverinfo['virtualserver_weblist_enabled'] = 0;
        }
