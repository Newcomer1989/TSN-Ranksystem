...
class DefaultExtension extends Base {
	...
	public function smarty_modifier_date_format($string, $format = null, $default_date = '', $formatter = 'auto')
	{
		...
		if ($formatter === 'strftime' || ($formatter === 'auto' && strpos($format, '%') !== false)) {
			...
			// Formatkonvertierung strftime → IntlDateFormatter
			$icuFormatMap = [
				'%a' => 'EEE',
				'%A' => 'EEEE',
				'%b' => 'MMM',
			...
			$icuFormat = $format;
			foreach ($icuFormatMap as $phpCode => $icuCode) {
				$icuFormat = str_replace($phpCode, $icuCode, $icuFormat);
			}

			if (!class_exists('IntlDateFormatter')) {
				throw new \RuntimeException('PHP-Intl extension required. Please enable ext-intl.');
				return @strftime($format, $timestamp);
			}
			$formatterObj = new \IntlDateFormatter(
				\Locale::getDefault(),
			...
			return $formatterObj->format($timestamp);
		} else {
